...
indir = glob.glob(
    f'/data/wsd01/st_helens_peter/dv/resp_removed_longtw_final_QCpass_ddt/xstations_{freq}-{freq*2}*')
...
stds = [2e-3*(3**n) for n in range(5)]
stds = np.hstack((stds, 0.256))
...
for std_model in stds:
    for corr_len in corr_lens:
        outdir = os.path.join(
            '/data/wsd01/st_helens_peter/spatial/ddv_newdv_dt_lcurve/')
        ...
        for utc in times[ind]:
            ...
            pred = dvg.forward_model(invma, dvs=dvs, utc=utc)
            ...
            residuals.append(np.sqrt(np.mean((real - pred)**2/sigma_d**2)))
...
if rank == 0:
    np.savez(
        os.path.join(outdir, f'Lcurve_{freq}.npz'), model_variances=np.array(mv),
