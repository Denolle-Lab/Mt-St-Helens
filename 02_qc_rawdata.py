...
Last Modified: Monday, 26th February 2024 03:20:18 pm
'''
import requests
import os
import glob
...
rms_file = os.path.join(proj_dir, '/rms/rms_{station}.npz')
...
mseed = os.path.join(
    proj_dir,
    '{year}/{network}/{station}/{channel}.D/{network}.{station}.{location}.{channel}.D.{year}.{jday}')
outfolder = os.path.join(proj_dir, 'figures', 'qc')
filterfolder = os.path.join(proj_dir, '/mseed_qcfail')
filtermseed = os.path.join(
    proj_dir,
    '/mseed_qcfail/{year}/{network}/{station}/{channel}.D/{network}.{station}.{location}.{channel}.D.{year}.{jday}')
corrdbs = os.path.join(
    proj_dir,
    '/corrs_response*/*/*{network}*.*{station}*.h5')
